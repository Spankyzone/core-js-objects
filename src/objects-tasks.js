...
function removeProperties(obj, keys) {
  const deleteKeysObj = obj;
  const arr = Object.keys(obj);
  if (Array.isArray(keys)) {
    for (let i = 0; i < arr.length; i += 1) {
      keys.forEach((element) => {
        if (element === arr[i]) {
          delete deleteKeysObj[element];
        }
      });
    }
  }
  delete deleteKeysObj[keys];
  return deleteKeysObj;
}
...
function compareObjects(obj1, obj2) {
  const keysObj1 = Object.values(obj1);
  const keysObj2 = Object.values(obj2);
  const areEqual = keysObj1.every((key, index) => key === keysObj2[index]);
  return areEqual;
}
...
function makeWord(lettersObject) {
  ...
  Object.keys(lettersObject)
    .flat()
    .forEach((letter) => {
      for (let i = 0; i < lettersObject[letter].length; i += 1) {
        word[lettersObject[letter][i]] = letter;
      }
    });
  return word.join('');
}
